...
<body>
    ...
    <main>
        <form method="POST" action="{{ route('remisiones.recibida') }}">
            ...
            @foreach ($tecnicas->groupBy('tipo_muestra_id') as $tipoId => $grupo)
                <div class="grupo-tecnicas" data-tipo="{{ $tipoId }}">
                    ...
                    @foreach ($grupo as $tecnica)
                        <div class="tecnica-card">
                            <div class="tecnica-header">
                                <label>
                                    <input type="checkbox" name="tecnicas[{{ $tecnica->id }}][id]"
                                        value="{{ $tecnica->id }}">
                            ...
                            <div style="margin-top:10px;">
                                Cantidad:
                                <input type="number" name="tecnicas[{{ $tecnica->id }}][cantidad]"
                                    class="form-control cantidad" min="0" value="0">
                                <button type="button" class="btn-animales"
                                    onclick="abrirModal({{ $tecnica->id }})">🐾 Asignar animales</button>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const tipoChecks = document.querySelectorAll('.chk-tipo-muestra');
            const grupos = document.querySelectorAll('.grupo-tecnicas');

            function actualizarVisibilidad() {
                const seleccionados = Array.from(tipoChecks)
                    .filter(c => c.checked)
                    .map(c => parseInt(c.value));

                grupos.forEach(grupo => {
                    const tipoId = parseInt(grupo.dataset.tipo);
                    grupo.style.display = (seleccionados.length === 0 || seleccionados.includes(tipoId)) ?
                        'block' :
                        'none';
                });
            }

            tipoChecks.forEach(check => check.addEventListener('change', actualizarVisibilidad));
